...
D["table_export.py<br/>table_md, tables_md"]
end
...
## 核心组件
- add_limit：构建单个条件片段，支持多种运算符、表别名、AND 前缀控制与特殊值短路
...
- table_md / tables_md：导出单表或多表结构为 Markdown 文档，包含字段、索引、字符集等信息
...
V --> |合法| T["检查表是否存在(LIKE)"]
T --> |不存在| E
T --> |存在| C1["查询字段信息(SHOW FULL COLUMNS)"]
C1 --> C2["查询主键信息(SHOW KEYS)"]
C2 --> C3["查询索引信息(SHOW INDEX)"]
C3 --> C4["查询表级字符集/排序规则(SHOW TABLE STATUS)"]
C4 --> Gen["生成Markdown内容"]
Gen --> W["写入文件/目录"]
W --> End(["结束"])
```
...
- SQLExecutor 在导出场景调用 table_export 的 table_md/tables_md
...
## 结论
lazy_mysql 的工具函数体系以“安全、灵活、易用”为核心设计原则，通过 add_limit、where_clause、fetch_format、table_export 等模块，覆盖了条件构建、参数化、结果格式化与文档导出的关键环节。它们既可独立使用，也能与 SQLExecutor 的 select/query 紧密协作，显著提升开发效率与可维护性。建议在实际项目中：
- 使用 where_clause 构建 WHERE 子句，确保参数化与类型安全
- 使用 fetch_format 控制输出格式，按需启用 DataFrame 与字典
- 使用 table_export 生成数据库文档，便于团队共享与审计
- 遵循扩展机制，按需新增表达式与输出格式
...
- 导出表结构
  ...
  - [lazy_mysql/tools/table_export.py:12-190](file://lazy_mysql/tools/table_export.py#L12-L190) 展示导出实现细节
